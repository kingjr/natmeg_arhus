function trl = artefact_trialfun_thresh(cfg)
...
if ~isfield(cfg, 'maxartfreq'), cfg.maxartfreq  = .200;                         end % max s between heart beat 
...
%-- mean (derivative on each pair of channel)
if length(cfg_ecg.channel) > 1, 
    if iseven(length(cfg_ecg.channel))
        data_ecg.trial{1} = nanmean(data_ecg.trial{1}((1:2:(end-1)),:) - data_ecg.trial{1}(2:2:end,:));
    else
        error('too many channels in artchan or odd number of channels!')
    end
end
%-- find pulse 
disp('finds artefact above thresh...');
...
if strcmp(cfg_ecg.plot, 'yes');
    ...
    scatter(data_ecg.time{1}(data_ecg.pulse),...
        ...
        length(data_ecg.pulse),1),'ro'); %  art beats
...
trl = [...
    ...
    repmat((cfg.poststim+cfg.prestim) .* data_ecg.fsample,1,length(data_ecg.pulse))]'; 

trl(end+1,2) = size(data_ecg.trial{1} ,2);% add info of data size
end