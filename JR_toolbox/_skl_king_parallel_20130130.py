...
# upadte    2013 11 26: correct TOI time generalization bug for single samples
...
if filenameX[-4:] == ".mat":  # backward compatibility
    ...
    # Xm_all = Xm_all.reshape(Xdim[0], Xdim[1], Xdim[2])
...
if "dims" in mat:
    ...
    dims = np.array(range(0,Xm_all.shape[2]))
...
def my_pipeline(train, test,
    ...
    dims, fs, scaler, clf,
    n_samples, n_dims, n_dims_tg, n_classes, wsize,
    fb_fold,fb_clf):
...
for split in range(n_splits):
    ...
    out = Parallel(n_jobs=n_cores)(delayed(my_pipeline)(
        ...
        wsize=wsize,
        fb_fold=fb_fold,
        fb_clf=fb_clf) for fold, (train, test) in enumerate(cv))
