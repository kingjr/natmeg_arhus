function [auc labels curve se] = colAUC(X, y, alg)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Adapted by JeanRémi King to:
%   - accept below .5 results 
%   - compute n-class standard errors
%   - deal with NaNs
%   - reshape for n-dimensional matrix
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (nL<=1)
  warning('colAUC: List of labels ''y'' have to contain at least 2 class labels.')
  [auc labels curve se] = deal(NaN);
else
...
switch alg
    case 'ROC'
        ...
        for c=1:nc                           % for each column representing a feature
            inan = find(~isnan(X(:,c)));
            [b, IDX] = sort( X(inan,c));          % sort all columns and store them in X. IDX stores original positions
            ...
            if (nTies<nr-1)                    % make sure all numbers in X column are not the same
                IDX = y(inan(IDX));            % reorder label vector in the same order as X, or associate label with each number in X
                % assign column for each label (class) and for each point add 1 in the column corresponding to its class
                d = ( IDX(:,ones(1,nL)) == L(inan,:));
  ...
  case 'Wilcoxon'
    ...
    for c = 1:nc                    % for each column representing a feature
      for i = 1:np                  % go through all permutations of columns in d
        c1 = per(i,1);              % and identify 2 classes to be compared
        c2 = per(i,2);
        inan = find(isnan(X([idxL{c1};idxL{c2}],c)));
        x1 = X(setdiff(idxL{c1},inan),c);
        x2 = X(setdiff(idxL{c2},inan),c);
        n1 = length(x1);
        n2 = length(x2);
        if (n1>0 && n2>0)
          r = avrRank([x1(:); x2(:)]);
...
%% estimate the area under the curve standard error of area using
...
for i = 1:np
    ...
    inan    = [];
    for c = nc:-1:1
        inan1(c) = sum(isnan(X(idxL{c1},c)));
        inan2(c) = sum(isnan(X(idxL{c2},c)));
    end
    lu      = sum(y==c1)-inan1;
    lh      = sum(y==c2)-inan2;
    auc2    = auc(i,:).^2;
    Q1      = auc(i,:)./(2-auc(i,:));
    Q2      = 2*auc2./(1+auc(i,:));
    V       = (auc(i,:).*(1-auc(i,:))+(lu-1).*(Q1-auc2)+(lh-1).*(Q2-auc2))./(lu.*lh);
    se(i,:) = realsqrt(V);
end
