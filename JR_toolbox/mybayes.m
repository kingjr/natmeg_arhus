...
class_prob = [sum(y==1) sum(y==2)] / length(y);
%-- parameters
if nargin == 3,cfg = []; end
if ~isfield(cfg,'prior') || ischar(cfg.prior), cfg.prior = class_prob; end % calculates probability of each class
%-- mean & std
for c = 1:2
...
predict     = NaN(length(post),1);
for ii = 1:length(post)
    predict(ii)= find(max(post(ii,:))==post(ii,:));                                   % predicted class
end
