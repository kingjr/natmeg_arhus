function ht = textbp(strings,xs,ys,p,varargin)
...
if nargin < 2
    xlim=get(gca,'Xlim'); %[data]
    xlim = xlim;
    xs = repmat((xlim(2)-xlim(1))/2,length(strings));
elseif nargin < 3
    ylim=get(gca,'Ylim'); %[data]
    ylim = ylim;
    ys = repmat((ylim(2)-ylim(1))/2,length(strings));
elseif nargin < 4
    p = [2 10];
end
...
h_temp = text(0,0,strings{1},'units','normalized',varargin{:});
...
for s = 1:length(strings)
    ...
    y = ys(s);
% do the hard work
pos = tscan(gca,width,height,TOL,x,y,p);
...
function [Pos]=tscan(ha,wdt,hgt,tol,x,y,p,stickytol,hl)
...
for yp=ylim(1):Thgt/2:(ylim(2)-Thgt),
    ...
    for xp=xlim(1):Twdt/2:(xlim(2)-Twdt),
        % calculate gravitation field
        field(j,i) = p(1)*sqrt((x-xp)^p(2) + (y-yp)^p(2));
        % calculate element field
        pop(j,i) = ...
